...
def _merge_results_from_chunks(results: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
    """
    对单个文档内所有 chunk 的抽取结果进行合并与去重，并计算初始度(degree)。
    """
    ...
    for name, entities_list in all_entities_grouped.items():
        ...
        main_entity['degree'] = len(entities_list)  # <--- NEW: 计算初始度
    ...
    for key, relations_list in all_relations_grouped.items():
        ...
        main_relation['degree'] = len(relations_list)  # <--- NEW: 计算初始度 (替换 weight)
        merged_relations.append(main_relation)

    return merged_entities, merged_relations


# ... extract_entities_and_relations 函数的其余部分保持不变 ...
async def extract_entities_and_relations(
    ...
    """
    从所有文本块中并发地抽取实体和关系，并对文档内的结果进行合并。
    """
    # <--- MODIFIED: 修正这里的类名以匹配导入 ---
    client = AsyncOpenAI(
    ...
    return doc_entities, doc_relations